...
- **Walltime / `estimates.expected_hs23_s`**: optional. When both
  `hepscore23_s_per_event` and `baseline_hs23_per_core` are set, walltime
  and expected work follow [HEPScore23](hepscore23.md) (shared
  ``file_common``). Packing stays size-based either way.
